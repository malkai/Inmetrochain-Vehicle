...
type Log struct {
	Timestamp_signal string `json:"timestamp_signal"`
	Log_details      string `json:"log_details"`
}
...
func (s *SmartContract) Invoke(stub shim.ChaincodeStubInterface) sc.Response {
	...
	} else if fn == "insertLog" {
		//retrieves the accumulated consumption
		return s.insertLog(stub, args)
...
func (s *SmartContract) insertLog(stub shim.ChaincodeStubInterface, args []string) sc.Response {

	//validate args vector lenght
	if len(args) != 2 {
		return shim.Error("It was expected 2 parameter: <ID> <sensor_log>")
	}

	//gets the parameter associated with the meter ID
	id := args[0]

	//try to convert the informed sensor log into the format []byte, required by Gomorph
	sensor_log, err := json.Marshal(args[1])
	...
	if sensor_log == nil {
		//sensor_data is not a proper number
		return shim.Error("Error on veryfing sensor_log, it is not a proper input")
	}

	MyLog := Log{}

	//convert bytes into a Data object
	json.Unmarshal(sensor_log, &MyLog)

	//update Data state in the ledger
	stub.PutState(id, sensor_log)

	//loging...
	fmt.Println("Updating Data consumption:", sensor_log)
...
func (s *SmartContract) queryHistory(stub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	for historyIer.HasNext() {
		//increments iterator
		queryHistory, err := historyIer.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
		...
		buffer.WriteString("\"")
		buffer.WriteString(queryHistory.TxId)
		buffer.WriteString("\"")
		buffer.WriteString(", \"Record\":")
		// Record is a JSON object, so we write as-is
		buffer.WriteString(string(queryHistory.Value))
		buffer.WriteString("}")
		bArrayMemberAlreadyWritten = true

		//increases counter
		counter++
		fmt.Printf(string(queryHistory.Value))
		fmt.Printf("Consulting ledger history, found %d\n records", counter)
	}
	buffer.WriteString("]")
