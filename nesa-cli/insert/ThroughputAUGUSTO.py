    ...
    def send_transactions(self):
        ...
        while not stop:
            try:
                ...
                c_hlf = client_fabric(net_profile=("../config/inmetro.br.json"))
                stop = True
            except:
                stop = False
...
if __name__ == "__main__":
    ...
    f = open('../mocks/mock_1.json')
    signal = json.load(f)
    signal = json.dumps(signal)
    data = json.loads(signal)

    id = data['device_id'] + data['signals'][0]['sensor_id']
    print(id)
    
    Sensor_data = { 
        'timestamp_signal': data['timestamp_signal'],  
        'sampling_period_in_sec': data['signals'][0]['sampling_period_in_sec'],
        'overall_samples:': data['signals'][0]['overall_samples'],
        'sample_rate_hz': data['signals'][0]['sample_rate_hz'],
        'total_of_seconds': data['signals'][0]['total_of_seconds'],
        'signal_data' : data['signals'][0]['signal_data']
    }
    ...
    processes = [mp.Process(target=multiproc,
                            args=(nthreads, 2000, threading.Lock()))
